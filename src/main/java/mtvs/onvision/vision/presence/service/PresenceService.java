...
public class PresenceService {
    ...
    public PresenceResponse getWardPresence(CurrentUser currentUser) {
        ...
        //상태값 없으면 연경x
        if (json.isEmpty())
            return new PresenceResponse(null, false, false);
        //있으면 연결상태 판별
        ...
        boolean isConnected = heartbeat.network().connected() && isRecent;
        return new PresenceResponse(heartbeat.battery(),  heartbeat.deviceConnected(), isConnected);
    }
}
