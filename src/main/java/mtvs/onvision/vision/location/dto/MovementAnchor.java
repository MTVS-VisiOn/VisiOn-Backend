 ...
 *
 * 기준점 좌표와 함께 {@link #vehicleStreak}도 들고 간다. 차량 판정은 한 번의 속도 계산으로
 * 확정하지 않고 연속 횟수를 세는데, 그 횟수가 보고 사이를 건너 살아남아야 하기 때문이다.
 */
public record MovementAnchor(
    ...
    /** 차량 연속 횟수를 이어받아 기준점만 옮긴다 */
    public static MovementAnchor of(LocationRequest request, int vehicleStreak) {
        return new MovementAnchor(request.latitude(), request.longitude(), request.accuracy(),
                request.recordedAt(), vehicleStreak);
    }
